...
(async()=>{
  ...
  assert.equal(doc.querySelector('#share-options').hidden,false);
  assert.equal(doc.querySelector('#share-short-url').value,shared);
  assert.match(doc.querySelector('#share-feedback').textContent,/Enlace preparado/);
  const whatsapp=new URL(doc.querySelector('#share-whatsapp').href),facebook=new URL(doc.querySelector('#share-facebook').href);
  ...
  assert.equal(doc.querySelector('#share-options').hidden,true);
  ...
  dom.window.close();
  console.log('Sharing DOM PASS: pending saves, revisions, privacy opt-ins, short links and revocation.');
