...
category_data = [{**selected, 'aliases': ['excavadora'], 'profile': EXCAVATOR_PROFILE if excavator else {}},
                 {**generic, 'aliases': ['montacargas'], 'profile': {}}]
machine=Machine(owner=user,title='Borrador de prueba',category_id=701,
                data={'hours':0,'brand':'Original','location':'Prueba, sin inventario','power':'100 kW',
                      **({'power_type': 'net'} if excavator else {})})
state={'id':str(machine.id),'revision':1,'title':machine.title,'category':701,'data':machine.data,'provenance':{},'editable':True,'status':'draft'}
