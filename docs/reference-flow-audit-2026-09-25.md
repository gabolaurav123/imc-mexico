...
| Implementado localmente | La política y pruebas locales fijan `gpt-6-luna` (`portal/ai_model.py`, `portal/tests/test_ai_model_policy.py`). La lectura de placas usa recortes acotados y valida cada campo de forma independiente. | Aún no se afirma una comprobación remota de credenciales, disponibilidad ni precisión. Antes de habilitar IA para visitantes, verificar en el entorno destino la configuración, worker y límites con una máquina autorizada; conservar el camino manual si falla. |
