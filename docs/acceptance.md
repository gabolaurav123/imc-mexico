...
| Código | [gabolaurav123/imc-mexico](https://github.com/gabolaurav123/imc-mexico), rama `main`; commit público verificado `535c6c6` |
...
| Analítica opcional | Código validado localmente y en PostgreSQL, pendiente del siguiente despliegue; `0005_optional_acquisition_analytics` ya aplicada en Neon |
...
| HTTPS | **72 de 72 comprobaciones aprobadas** | Registro del 15 de septiembre, 23:26:51–23:37:17 UTC |
...
| Persistencia | JPEG, HEIC, MOV, ficha y análisis conservados tras redespliegue `535c6c6` | Lectura real de objetos y datos anteriores al redespliegue |
...
| Restauración | PostgreSQL y **10 archivos** restaurados y verificados en `imc_restore_test` | Entorno aislado; daemon programado en verificación y sin copia externa S3 |

El registro saneado `public-qa-evidence.json` del workspace contiene las 72 comprobaciones HTTP, sin contraseñas ni tokens. Las verificaciones de proveedor, Gmail, restauración y navegador complementan ese registro; no se suman como pruebas automatizadas del mismo tipo. Detalles técnicos: [processing.md](processing.md), [operations.md](operations.md) y [ui-validation.md](ui-validation.md).
...
### Analítica opcional, validada antes del despliegue
...
- `retention --apply` limpia hashes de sesión de más de 30 minutos y contexto de análisis expirado; la aplicación sigue siendo manual. Los 19 tests de privacidad y las cinco comprobaciones PostgreSQL pasaron; falta verificar este cambio en el servicio publicado.
...
La persistencia del servicio se comprobó separadamente: JPEG, HEIC, MOV y resultados de IA anteriores siguieron disponibles después del redespliegue `535c6c6`. Neon tiene aplicadas `0001`–`0005`. El código de analítica de `0005` sigue pendiente de despliegue; sus cinco comprobaciones reales en PostgreSQL pasaron con rollback: datos anónimos sin identidad, finalización asíncrona idempotente, revocación de hash/contexto, rechazo en modo agregado y expiración del contexto JSON.
...
La corrección de contenido público y logout se verificaron sobre `535c6c6`. La sesión del navegador se interrumpió antes de repetir la captura móvil del ajuste de espacios en títulos y de inspeccionar visualmente el aviso de registro cerrado. Ese cierre sí está cubierto por HTTP y servidor. Las pruebas DOM de autosave, reintentos y conflictos están delimitadas en [ui-validation.md](ui-validation.md).

### Copia y restauración

Se produjo y restauró una copia real de PostgreSQL y medios en `imc_restore_test`, con verificación de **10 archivos**. No se restauró encima de la base operativa ni sobre proyectos ajenos. Esta prueba acredita el formato y recuperación aislada realizada.

El daemon programado y su estado de último éxito en el volumen de producción siguen en verificación. No existe destino externo S3 configurado/verificado. Una copia junto a los medios no protege ante pérdida del volumen de 5 GB, y la aplicación no cifra la copia local. Falta validar la recuperación integral ante pérdida del servicio/volumen. Procedimientos: [operations.md](operations.md).
...
4. **Respaldo:** confirmar ejecución programada, configurar copia externa privada y probar recuperación integral. Revisar capacidad conjunta de medios y copias en 5 GB.
5. **Analítica opcional:** desplegar el código ya validado y comprobar preferencias, origen/campaña y exportación desde la URL pública antes de declararlos activos. `0005` ya está aplicada; la analítica viene desactivada por defecto.
